...
while True:
    try:
        ...
        for url in urls:
            notify_slack_url_activity(url=url, user=default_user, token=default_token, pickle='urls', channel='test', hook=default_hook)
        ...
        notify_slack_group_activity(group='8gk9i7VV', groupname='Anyone Can Join', user=default_user, token=default_token, pickle='AnyoneCanJoin', channel='test', hook=default_hook)

        # notify when there's an annotation with a specified tag
        notify_slack_tag_activity(tag='nextprez', user=default_user, token=default_token, pickle='nextprez', channel='test', hook=default_hook)
